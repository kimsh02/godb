...
func NewHeapFile(fromFile string, td *TupleDesc, bp *BufferPool) (*HeapFile, error) {
	...
	return &HeapFile{td, int(numPages), fromFile, -1, bp, DebugMutex{sync.Mutex{}}}, nil
...
func (f *HeapFile) NumPages() int {
	...
	// fmt.Println("in NumPages()")
	// f.mu.Lock(tid)
	// defer f.mu.Unlock(tid)
	return f.numPages
}
...
func (f *HeapFile) insertTuple(t *Tuple, tid TransactionID) error {
	f.mu.Lock(tid, "insertTuple")
	// fmt.Printf("lock from insertTuple, tid: %d\n", tid)
	defer f.mu.Unlock(tid, "insertTuple")
	...
	for p := start; p < endPage; p++ {
		// fmt.Println("getting page to read from insertTuple")
		// f.mu.Lock(tid, "insertTuple")
		pg, err := f.bufPool.GetPage(f, p, tid, ReadPerm)
		// f.mu.Unlock(tid, "insertTuple")
		if err != nil {
			return err
		}
		...
		// fmt.Println("getting page to write from insertTuple")
		// f.mu.Lock(tid, "insertTuple")
...
func (f *HeapFile) deleteTuple(t *Tuple, tid TransactionID) error {
	f.mu.Lock(tid, "deleteTuple")
	defer f.mu.Unlock(tid, "deleteTuple")
	// fmt.Printf("lock from deleteTuple, tid: %d\n", tid)
	...
	// fmt.Println("unlocked from numPages from deleteTuple")

	// fmt.Printf("deleteTuple calling getPage, tid %d\n", tid)

	// f.bufPool.mu.Lock(tid)

	// f.mu.Lock(tid, "deleteTuple")

	pg, err := f.bufPool.GetPage(f, rid.pageNo, tid, WritePerm)

	// f.mu.Unlock(tid, "deleteTuple")

	// f.bufPool.mu.Unlock(tid)
