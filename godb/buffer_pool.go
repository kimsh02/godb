...
func (bp *BufferPool) releaseLocks(tid TransactionID) {
	...
	// fmt.Printf("released locks for tid: %d\n\n", tid)
	// for k := range bp.locks {
	// 	fmt.Println(k)
	// 	fmt.Println(bp.locks[k])
	// }
...
func (bp *BufferPool) acquireLock(pageId any, tid TransactionID, perm RWPerm) error {
	...
	for {
		...
		fmt.Println()
		fmt.Printf("lock from acquireLock, for ")
		fmt.Print(pageId)
		fmt.Printf(", permission %d, tid %d\n", perm, tid)
		fmt.Printf("current locks, tid %d:\n", tid)
		for k := range bp.locks {
			fmt.Println(k)
			fmt.Println(bp.locks[k])
		}
		fmt.Printf("running transactions, tid %d:\n", tid)
		fmt.Println(bp.running)
		...
		if perm == ReadPerm && (lock.exclusive == nil || *lock.exclusive == tid) {
			...
			fmt.Printf("shared lock granted, tid: %d\n", tid)
			fmt.Println()
			for k := range bp.locks {
				fmt.Println(k)
				fmt.Println(bp.locks[k])
		...
		if _, ok := lock.shared[tid]; (ok && len(lock.shared) == 1 ||
			...
			fmt.Printf("exclusive lock granted, tid: %d\n", tid)
			fmt.Println()
			for k := range bp.locks {
				fmt.Println(k)
				fmt.Println(bp.locks[k])
		...
		fmt.Printf("not able to grant lock yet, tid: %d\n", tid)
		...
		if bp.detectDeadlock(tid) {
			...
			fmt.Printf("deadlock occurred, tid: %d\n", tid)
		...
		// if ctr > 5 || len(bp.wfg) > 10 {

		// 	fmt.Printf("timeout occurred, tid: %d\n", tid)
		// 	for i := range bp.wfg {
		// 		for j := range bp.wfg[i] {
		// 			bp.mu.Unlock(tid, "acquireLock")
		// 			bp.CommitTransaction(j)
		// 			bp.mu.Lock(tid, "acquireLock")
		// 		}
		// 		// bp.mu.Unlock(tid, "acquireLock")
		// 		// bp.AbortTransaction(i)
		// 		// bp.mu.Lock(tid, "acquireLock")
		// 	}
		// 	bp.mu.Unlock(tid, "acquireLock")
		// 	return nil
		// 	// for k := range bp.wfg[tid] {
		// 	// 	bp.mu.Unlock(tid, "acquireLock")
		// 	// 	bp.AbortTransaction(k)
		// 	// 	return nil
		// 	// }
